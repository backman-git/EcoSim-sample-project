...
//  HAL_ENTER_CRITICAL_SECTION( intState );  // Hold off interrupts.
  ...
  if ((osalMemStat == 0) || (size <= OSALMEM_SMALL_BLKSZ))    //BX OSALMEM_SMALL_BLKSZ 16 byte
  {
    hdr = ff1;
 ...
 */
 ...
 //BX
/*
  uartWriteString("==mem matrix==\n\r");
  uartWriteU16(blkMax);
  uartWriteString("\n\r");
  uartWriteU16(blkCnt);
  uartWriteString("\n\r");
  uartWriteU16(blkFree);
  uartWriteString("\n\r");
  uartWriteU16(memAlo);
  uartWriteString("\n\r");
  uartWriteU16(memMax);
