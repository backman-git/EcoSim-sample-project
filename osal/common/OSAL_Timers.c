...
osalTimerRec_t * osalAddTimer( uint8 task_id, uint16 event_flag, uint32 timeout )
{
  ...
  else
  {
    ...
    {
   //   uartWriteString("NNONONONONONONONONO\n\r");
...
void osalTimerUpdate( uint32 updateTime )
{
 ...
 // HAL_EXIT_CRITICAL_SECTION( intState );   // Re-enable interrupts.
  ...
  if ( timerHead != NULL )
  {
    ...
    /*
        uartWriteString("head value: ");
        uartWriteU16(timerHead->task_id);
        uartWriteString("\r\n");
        uartWriteU16(srchTimer->task_id);
        uartWriteString("\r\n");
        uartWriteU16(timerHead->event_flag);
        uartWriteString("\r\n");
        uartWriteU16(srchTimer->event_flag);
        uartWriteString("\n\r");
*/
...
/*
        uartWriteString("sys clock::: ");
        uartWriteU16(osal_systemClock);
        uartWriteString("\r\n");
        uartWriteString("srchTimer value: ");
        uartWriteU16(srchTimer->task_id);
        uartWriteString("\r\n");
        uartWriteU16(srchTimer->event_flag);
        uartWriteString("\n\r");
   ...
   //   HAL_EXIT_CRITICAL_SECTION( intState );   // Re-enable interrupts.
      ...
      if ( freeTimer )
      {
       ...
       /*   
          uartWriteString("/n/r");
          uartWriteString("ID: ");
          uartWriteU16(freeTimer->task_id);
          uartWriteString("/n/r");
          uartWriteString("flag: ");
          uartWriteU16(freeTimer->event_flag);
          uartWriteString("/n/r");
*/
